...
```sh
$ docker run -it --rm -v $(pwd):/tmp/tst --user ubuntu --device=/dev/ttyUSB0 --device=/dev/ttyUSB1 espressif/idf:v5.5.3
...
```sh
docker run -it --rm --user ubuntu --device=/dev/ttyUSB0 --device=/dev/ttyUSB1 -v $(pwd):/tmp/tst espressif/idf:v5.5.3
cd /tmp/tst/firmware
idf.py set-target esp32s3
idf.py build
idf.py -p /dev/ttyUSB1 flash
